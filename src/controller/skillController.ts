      ...
      if(!isUserExist){
          ...
          req.body.qualifications.forEach(async (q:any) => {
              if(q._id===''){
                  ...
                  if(q.name==="" && q.source==="" && q.acqdate===null){
...
router.post("/deleteEngineer", async (req, res) => {
    ...
    try {
      ...
      await user.findOneAndUpdate({ userid: userId,delflg: 0 }, update);
      await userinfo.findOneAndUpdate({ userid: userId,delflg: 0 }, update);
      await certification.updateMany({ userid: userId,delflg: 0 }, update);
      await specexp.updateMany({ userid: userId,delflg: 0 }, update);
      await jobhistory.updateMany({ userid: userId,delflg: 0 }, update);
      await usertag.deleteMany({ userid : userId,delflg: 0 });
