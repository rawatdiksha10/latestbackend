...
router.post('/search', async (req, res) => {
  ...
  try {
    ...
    if (jpLevel){
        ...
        if (expOver || expUnder) {
          ...
          if(certificate) {
            ...
            if (language.length> 0) {
              ...
              const UserTagResult = await findUserTag(certificateUserIds,labelIds);
  
              const userTagUserIds = UserTagResult.flat().map((result: { _id: any; }) => result._id);

              if (searchKey) {
                const filteredUserIds = userTagUserIds.filter((userId: any) => userIdsByKW.includes(userId));
            ...
            const UserTagResult = await findUserTag(specExpUserIds,labelIds);
          ...
          const UserTagResult = await findUserTag(jpLevelUserIds,labelIds);
      ...
      const userQueries = await findUserTag([],labelIds);

      //const userQueries = await UserTag.find({ lbltgid: { $in: labelIds } });
      userIds = userQueries.flat().map((result: { _id: any; }) => result._id);
...
async function findUserTag(userIds:any, labelIds: any): Promise<any> {
  try {

    let userTagResult:any;

    if (userIds.length > 0) {

      userTagResult = await UserTag.aggregate([
        {
          $group: {
            _id: "$userid",
            lbltgid: { $push: "$lbltgid" }
          }
        },
        {
          $match: {
            $and: [
                {_id: { $in: userIds } },
                {lbltgid: { $all: labelIds }}
              ]
          }
        }
      ]);
      console.log('with userid',userTagResult);
    } else {
      ...
      console.log('without userid',userTagResult);
    }
